#!/usr/bin/env python3
"""
Script to generate custom exercises for all 185 Linux Journey lessons.
Each exercise is tailored to the specific lesson content.
"""
import os
import re
...
def get_exercise_content(filename):
    """Return custom exercise content for each lesson file."""
    
    exercises = {
        # Command Line lessons (already done - 19 files)
...
2. **Repeat a specific command**: Run command number from history
   ```bash
   !-2
   ```
   Expected output:
   ```
   (Executes the second-to-last command)
   ```
...
4. **Compare with man pages**: See the difference between help and man
   ...
   Expected output:
   ```
   (help shows brief info for shell built-ins)
   ```""",
...
1. **Read the manual for ls**: Open the comprehensive manual page
   ...
   Expected output:
   ```
   (Opens manual page - use arrow keys to navigate, press 'q' to quit)
   ```
...
3. **View manual for a configuration file**: Check documentation for passwd file
   ...
   Expected output:
   ```
   (Opens manual page for passwd file format - section 5)
   ```
...
4. **Compare with man -k**: See the difference between whatis and man -k
   ```bash
   whatis ls
   man -k "list directory" | head -3
   ```
   Expected output:
   ```
   (whatis shows brief summary, man -k shows more results)
   ```""",
...
5. **Clean up**: Remove test directory
   ...
   rmdir tempdir
   ```
   Note: Type `exit` to close your terminal session when ready""",
...
def main():
    ...
    for filepath in all_files:
        ...
        exercise = get_exercise_content(filename)
        
        if exercise:
            try:
                update_lesson_file(filepath, exercise)
                print(f"✓ Updated: {filepath.relative_to(lessons_dir)}")
                updated_count += 1
